...
function mainapi:CreateGUI()
    ...
    if mainapi.ThreadFix then
        ...
        Main.Parent = cloneref(game:GetService('Players')).LocalPlayer.PlayerGui
    ...
    self.AddCatalog = function(self,arg)
        ...
        Catalog.AddModule = function(self,arg)
            ...
            mainapi:Clean(CatalogNameText:GetPropertyChangedSignal('AbsoluteSize'):Connect(function()
                -- TweenService:Create(MainFrame,TweenInfo.new(0.05, Enum.EasingStyle.Exponential),
                --     {Size = UDim2.new(1, 0, 0, CatalogNameText.AbsoluteSize.Y*0.33)}
                -- ):Play()
                if mainapi.ClickGuiStatus then
                    MainFrame.Size = UDim2.new(1, 0, 0, CatalogNameText.AbsoluteSize.Y * 0.33)
                    local Size = 0
            ...
            if Module.Enabled then
                TweenService:Create(MainButton,TweenInfo.new(0.3, Enum.EasingStyle.Exponential),
            ...
            function Module:Delete()
                ...
                table.clear(Module.Connections)
                if old then
                    task.spawn(Module.Function, false)
                end
            ...
            function Module.getChilrenSize()
                ...
                if Size == CatalogNameText.AbsoluteSize.Y*0.05 then
                    return 0
                end
                return Size
            end
            ...
            function Module:Expand()
                ...
                TweenService:Create(Children, TweenInfo.new(0.2, Enum.EasingStyle.Exponential),
                    {Size = Module.Expanded and UDim2.new(1, 0, 0, Module.getChilrenSize()) or UDim2.new(1, 0, 0, 0)}
                ):Play()
                for i,v in next, Children:GetDescendants() do
                    if v:IsA('Frame') and v.Name == 'Slider_Fill' then
                        if Module.Expanded then
                            v.Size = UDim2.fromScale(0, 1) 
                            TweenService:Create(v, TweenInfo.new(0.7, Enum.EasingStyle.Exponential),
                                {Size = UDim2.fromScale(v:GetAttribute('Value'), 1)}
                            ):Play()
                        end
                    end
                end
            ...
            mainapi:Clean(UIListLayout:GetPropertyChangedSignal('AbsoluteContentSize'):Connect(function()
                if mainapi.ClickGuiStatus then
                    if ClickGui.UIScale.Scale == mainapi.Scale.Value or ClickGui.UIScale.Scale == 0 then
                        TweenService:Create(Children, TweenInfo.new(0.1, Enum.EasingStyle.Exponential),
                            {Size = Module.Expanded and UDim2.new(1, 0, 0, Module.getChilrenSize()) or UDim2.new(1, 0, 0, 0)}
                        ):Play()
                    end
            ...
            Module.AddToggle = function(self,arg)
                ...
                if Toggle.Enabled then
                    TweenService:Create(ToggleButtonFill,TweenInfo.new(0.5, Enum.EasingStyle.Exponential),
                        {Position = Toggle.Enabled and UDim2.fromScale(0.75, 0.5) or UDim2.fromScale(0.25, 0.5)}
                    ):Play()
                    TweenService:Create(GlowEffect,TweenInfo.new(0.3, Enum.EasingStyle.Exponential),
                        {Size = Toggle.Enabled and UDim2.fromScale(3, 3) or UDim2.fromScale(0, 0)}
                    ):Play()
                ...
                function Toggle:Load(tab)
                    if tab and Toggle.Enabled ~= tab.Enabled then
                        Toggle:Toggle(tab.Enabled)
                    end
                end
            ...
            Module.AddSlider = function(self, arg)
                ...
                local function SetValue(val, val2, final, skip)
                    ...
                    if not skip then
                        TweenService:Create(SliderLine3, TweenInfo.new(0.7, Enum.EasingStyle.Exponential),
                ...
                function Slider:Load(tab)
                    if
                        tab and tab.Max and Slider.Min and
                        tab.Max == Slider.Max and tab.Min == Slider.Min and
                        tab.Value
                    then
            ...
            Module.AddDropdown = function(self,arg)
                ...
                mainapi:Clean(MainButton:GetPropertyChangedSignal('AbsoluteSize'):Connect(function()
                    if mainapi.ClickGuiStatus then
                        DropdownFrame.Size = UDim2.new(1, 0, 0, MainButton.AbsoluteSize.Y * 0.8)
                        -- TweenService:Create(DropdownFrame,TweenInfo.new(0.1, Enum.EasingStyle.Exponential),
                        --     {Size = UDim2.new(1, 0, 0, MainButton.AbsoluteSize.Y * 0.8)}
                        -- ):Play()
                    end
                end))
    ...
    mainapi:Clean(RunService.PreSimulation:Connect(function()
        if Array.Enabled then
            for i,v in next, ArrayList:GetChildren() do
                if v:IsA("Frame") then
                    -- v.TextLabel.TextColor3 = mainapi:GetArrayListColor(v.AbsolutePosition)
	...
	UpdateInfo = function(self)
		...
		if v ~= nil or mainapi.ClickGuiStatus and UIScale.Scale == 0 and not Tween then
            Tween = TweenService:Create(UIScale,TweenInfo.new(0.1,Enum.EasingStyle.Bounce),
                {Scale = TargetHudScale.Value or 1}
            ):Play()
	...
	if not UserInputService:GetFocusedTextBox() and inputObj.KeyCode ~= Enum.KeyCode.Unknown then
		if table.find(mainapi.Keybind, inputObj.KeyCode.Name) then
            ...
            TweenService:Create(mainapi.TargetHudFrame.Frame.UIScale,TweenInfo.new(0.3,Enum.EasingStyle.Bounce),
                {Scale = mainapi.ClickGuiStatus and (TargetHudScale.Value or 1) or 0}
            ):Play()
		end
	end
...
function mainapi:Load()
    ...
    if isfile('Modern/Config/'..self.Place..'.txt') then
		...
		for i, v in savedata.Modules do
            ...
            local keycode
            pcall(function()
                keycode = Enum.KeyCode[v.Bind]
            end)
            if keycode then
                object:SetBind(keycode)
            end
		end
    end
...
run(function()
    ...
    local List = {}
    for i,_ in next, ThemeList do
        table.insert(List, i)
    end
    Theme = Interface:AddDropdown({
        Name = 'Theme',
        List = List,
        Default = "Shadow",
        Function = function(Selected)
            if ThemeList[Selected] then
                uipallet.MainColor = ThemeList[Selected][1]
                uipallet.SecondaryColor = ThemeList[Selected][2]
                uipallet.ThirdColor = ThemeList[Selected][3]
            end
        end
    })
...
run(function()
    TargetHud = Other:AddModule({
        Name = "Target Hud",
        Function = function(callback)
            if callback then
                TargetHud:Clean(mainapi.MainScreenGui:GetPropertyChangedSignal('AbsoluteSize'):Connect(function()
                    TargetHudFrame.Size = UDim2.new(0, mainapi.MainScreenGui.AbsoluteSize.Y/6, 0, mainapi.MainScreenGui.AbsoluteSize.Y/14)
                end))
                TargetHud:Clean(RunService.PreSimulation:Connect(function()
                    Targetinfo:UpdateInfo()
                end))
                TargetHudMain.Visible = true
                UIScale.Scale = 0
                TweenService:Create(UIScale,TweenInfo.new(0.3,Enum.EasingStyle.Bounce),
                    {Scale = TargetHudScale.Value or 1}
                ):Play()
            else
                UIScale.Scale = TargetHudScale.Value or 1
                TweenService:Create(UIScale,TweenInfo.new(0.3,Enum.EasingStyle.Bounce),
                    {Scale = 0}
                ):Play()
                task.wait(0.3)
    ...
    TargetHudSoundEffect = TargetHud:AddToggle({
        Name = "Sound Effect"
    })
    TargetHudScale = TargetHud:AddSlider({
        Name = "Scale",
        Min = 0.1,
        Max = 3,
        Decimal = 10,
...
run(function()
    Array = Other:AddModule({
        Name = "ArrayList",
        Function = function(callback)
            ArrayList.Visible = callback
        end
    })
    ArrayListTransparency = Array:AddSlider({
		Name = 'Transparency',
		Min = 0,
		Max = 1,
		Function = function(val)
			for _, Frame in ArrayList:GetChildren() do
                if Frame:IsA("Frame") then
                    Frame.TextLabel.BackgroundTransparency = 1 - val
                end
			end
		end,
		Decimal = 10,
    ...
    ArrayListGlowLine = Array:AddToggle({
		Name = 'Glow Line',
		Function = function(callback)
			for _, Frame in ArrayList:GetChildren() do
                if Frame:IsA("Frame") then
                    Frame.TextLabel.Frame.Visible = callback
                end
			end
		end
	})
